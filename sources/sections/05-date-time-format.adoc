...
Since the format can include all sorts of additional information in the suffix,
different standards bodies/organizations need a way to identify which part
adheres to their standards. For this, all information needs to be namespaced.
All information is therefore divided into three hyphen-separated sections: the
namespace, the key and the value. For example, the calendar as defined by the
Unicode consortium could be included as `u-ca-chinese`.

Currently, two namespaces are supported:

* `u` for the Unicode Consortium, the different keys accepted under this
namespace are standardized by the Unicode Consortium.
* `x` for Private Use, any keys can be used with this namespace, it is not
standardized.
...
calendar       = "[u-ca-" calendar-id "]"

suffix-part    = "[" ALPHA "-" 2*ALPHA "-" 1*VCHAR "]"
